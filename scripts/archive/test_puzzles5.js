async function onRequestGet(dateOverride) {
    const CATEGORIES = ['Sports', 'Landmarks', 'History', 'Entertainment', 'Business', 'Technology', 'Cinema'];
...
const FALLBACK_DATA = {
    ...
    for (let i = 0; i < 60; i++) {
        ...
        const selected = shuffleSeeded([...categoryList], randomFn).slice(0, 7);
...
async function test() {
    ...
    let p1 = data1.find(p => p.date === "2026-07-07");
    
    let res2 = await onRequestGet("7/7/2026, 1:27:15 PM");
    let data2 = await res2.json();
    let p2 = data2.find(p => p.date === "2026-07-07");
